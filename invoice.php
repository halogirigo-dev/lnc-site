...
// ── Load booking: DB → session → demo ─────────────────────────
$url_ref = preg_replace('/[^A-Z0-9\-]/', '', strtoupper(trim($_GET['ref'] ?? '')));

$db_booking  = $url_ref ? lnc_get_booking($url_ref) : null;
$db_payments = $db_booking ? lnc_get_payments($url_ref) : [];
...
if ($db_booking) {
  $b    = $db_booking;
  $from = 'db';
} elseif (isset($_SESSION['lnc_booking'])) {
  $sess = $_SESSION['lnc_booking'];
  if (!$url_ref || $url_ref === ($sess['ref'] ?? '')) {
    $b    = $sess;
    $from = 'session';
  }
}

// Build normalised invoice array
if ($b && $from !== 'demo') {
  $ref = $b['ref'] ?? $url_ref;

  if ($from === 'db') {
    $pkg_price  = $b['package_price_per_pax'] ?? 0;
    $guests_num = (int)($b['guests'] ?? 1);
    $total      = (int)($b['total_amount'] ?? 0);
    $deposit    = (int)($b['deposit_amount'] ?? 0);
    $balance    = (int)($b['balance_amount'] ?? 0);
    $has_price  = $total > 0;
    $status_db  = $b['status'] ?? 'pending_payment';
    $items      = [[
    ...
    $dep_payment = null;
    $bal_payment = null;
    foreach ($db_payments as $p) {
      if ($p['payment_type'] === 'deposit') $dep_payment = $p;
      if ($p['payment_type'] === 'balance') $bal_payment = $p;
    }
    $dep_paid = $dep_payment && in_array($dep_payment['midtrans_status'], ['settlement','capture']);
    $bal_paid = $bal_payment && in_array($bal_payment['midtrans_status'], ['settlement','capture']);

  } else {
    // Session
    $pkg_price  = $b['package_price'] ?? 0;
    $guests_num = (int)($b['guests'] ?? 1);
    $total      = (int)($b['subtotal'] ?? $b['total_amount'] ?? 0);
    $deposit    = (int)($b['deposit'] ?? $b['deposit_amount'] ?? 0);
    $balance    = (int)($b['balance'] ?? $b['balance_amount'] ?? 0);
    $has_price  = $pkg_price > 0;
    $status_db  = 'pending_payment';
    $dep_paid   = false;
    $bal_paid   = false;
    $items      = [[
      'desc'   => ($b['package_title'] ?? '') . ' — ' . ($b['package_duration'] ?? ''),
  ...
  $invoice = [
    ...
    'agent'         => 'Arief Hidayat',
    'issued'        => isset($b['created_at']) ? date('d M Y', strtotime($b['created_at'])) : date('d M Y'),
    'expiry'        => date('d M Y', strtotime('+14 days')),
    'due_deposit'   => date('d M Y', strtotime('+7 days')),
    'due_balance'   => !empty($b['dates']) ? 'Before departure' : date('d M Y', strtotime('+30 days')),
    'deposit_pct'   => 30,
    'has_price'     => $has_price,
    'items'         => $items,
    'total'         => $total,
    'deposit'       => $deposit,
    'balance'       => $balance,
    'dep_paid'      => $dep_paid ?? false,
    'bal_paid'      => $bal_paid ?? false,
  ];
} else {
  // Demo data
  $invoice = [
...
// Status badge config
$status_cfg = match($invoice['status'] ?? 'pending_payment') {
  'deposit_paid'             => ['Deposit Confirmed', '#2cb896', '#f0faf7'],
  'balance_paid', 'confirmed'=> ['Fully Paid ✓',     '#2a7a52', '#f0fdf4'],
  'cancelled'                => ['Cancelled',         '#e53935', '#fef2f2'],
...
<div class="no-print" style="background:#fff;border-bottom:1px solid rgba(0,0,0,.08);padding:0 48px;height:72px;display:flex;align-items:center;justify-content:space-between;margin-top:72px;">
  <div style="display:flex;align-items:center;gap:24px;">
    ...
    <span style="display:inline-block;padding:4px 14px;font-family:'MuseoModerno',sans-serif;font-weight:700;font-size:10px;letter-spacing:.12em;text-transform:uppercase;background:<?= $status_cfg[2] ?>;color:<?= $status_cfg[0] ?>;border:1px solid <?= $status_cfg[1] ?>;"><?= htmlspecialchars($status_cfg[0]) ?></span>
  </div>
  <div style="display:flex;gap:8px;">
    <button id="btn-print" class="btn btn--dark btn--sm" onclick="window.print()">↓ Download PDF</button>
...
<div class="no-print" style="background:#fff;border-bottom:1px solid rgba(0,0,0,.08);padding:0 48px;">
  <div style="display:flex;max-width:900px;margin:0 auto;">
    ...
    <button class="inv-tab-btn" data-idx="<?= $i ?>" style="padding:16px 24px;font-family:'MuseoModerno',sans-serif;font-weight:700;font-size:11px;letter-spacing:.14em;text-transform:uppercase;cursor:pointer;border:none;background:transparent;color:#8a7d6e;border-bottom:2px solid transparent;text-align:left;transition:all .2s;">
...
<div style="padding:48px 24px;background:#ede9e1;">
    ...
    <div class="doc-wrap">
      ...
      <div class="doc-guest-block">
        ...
        <div class="doc-guest-col">
          ...
          <p style="font-family:'Museo',sans-serif;font-size:13px;color:#8a7d6e;"><?= htmlspecialchars($invoice['duration']) ?> · <?= $invoice['guests'] ?> guest(s)</p>
      ...
      <div style="background:#f0faf7;padding:20px 24px;border-left:3px solid #2cb896;margin:24px 0;">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
          <div>
            <p style="font-family:'Museo',sans-serif;font-size:13px;color:#8a7d6e;">Deposit (30%) to confirm booking</p>
          ...
          <div>
            <p style="font-family:'Museo',sans-serif;font-size:13px;color:#8a7d6e;">Balance (70%) before departure</p>
      ...
      <div style="display:flex;justify-content:space-between;align-items:center;margin-top:24px;padding-top:24px;border-top:1px solid #e0d8ce;">
        ...
        <div style="text-align:right;">
          <?php if ($invoice['has_price'] && !$invoice['dep_paid']): ?>
          <a href="payment.php?ref=<?= urlencode($invoice['ref']) ?>" class="btn btn--primary">Pay Deposit →</a>
          <?php elseif ($invoice['dep_paid']): ?>
          <a href="https://wa.me/<?= SITE_WA ?>?text=<?= urlencode("Hi LNC, deposit confirmed. Ref: {$invoice['ref']}") ?>" target="_blank" class="btn btn--primary">WhatsApp Us →</a>
          <?php else: ?>
          <a href="https://wa.me/<?= SITE_WA ?>?text=<?= urlencode("Hi, I'd like to discuss pricing. Ref: {$invoice['ref']}") ?>" target="_blank" class="btn btn--primary">Discuss Pricing →</a>
          <?php endif; ?>
        </div>
      </div>
    </div>
  ...
  <div class="inv-tab-panel" style="display:none;">
    <div class="doc-wrap">
      <div class="doc-header">
        <div style="display:flex;align-items:center;gap:14px;">
          <img src="<?= UPLOADS_URL ?>/logo-1777215811265.png" style="height:48px;" alt="LNC">
        ...
        <div style="text-align:right;">
          ...
          <div style="font-family:'Museo',sans-serif;font-size:12px;color:#8a7d6e;margin-top:4px;">Issued: <?= $invoice['issued'] ?></div>
          <?php if ($invoice['dep_paid']): ?>
          <div style="font-family:'MuseoModerno',sans-serif;font-weight:700;font-size:12px;color:#2cb896;margin-top:4px;">PAID ✓</div>
          <?php else: ?>
      ...
      <?php if ($invoice['has_price']): ?>
      <div class="doc-table-head" style="display:grid;grid-template-columns:1fr auto auto;">
        <span>Description</span><span style="text-align:right;">%</span><span style="text-align:right;min-width:140px;">Amount</span>
      </div>
      <div style="background:#fff;padding:20px;border-bottom:1px solid #f0ebe3;display:grid;grid-template-columns:1fr auto auto;gap:24px;align-items:center;">
        <div>
          ...
          <p style="font-family:'Museo',sans-serif;font-size:12px;color:#8a7d6e;margin-top:2px;">Ref: PRO-<?= htmlspecialchars($invoice['ref']) ?></p>
        </div>
        <p style="font-family:'Museo',sans-serif;font-size:13px;color:#8a7d6e;text-align:right;"><?= $invoice['deposit_pct'] ?>%</p>
        <div style="text-align:right;min-width:140px;">
          <p style="font-family:'MuseoModerno',sans-serif;font-weight:900;font-size:20px;color:#1a2118;">Rp <?= number_format($invoice['deposit'],0,',','.') ?></p>
          <?php if ($invoice['dep_paid']): ?>
          <span style="font-family:'MuseoModerno',sans-serif;font-weight:700;font-size:11px;color:#2cb896;">PAID ✓</span>
          <?php else: ?>
          <span style="font-family:'MuseoModerno',sans-serif;font-weight:700;font-size:11px;color:#c4964a;">UNPAID</span>
          <?php endif; ?>
        </div>
      </div>
      ...
      <div style="background:#faf7f3;padding:16px 20px;display:flex;justify-content:space-between;align-items:flex-end;">
        <div>
          <p style="font-family:'Museo',sans-serif;font-size:12px;color:#8a7d6e;">Balance: Rp <?= number_format($invoice['balance'],0,',','.') ?> · due <?= $invoice['due_balance'] ?></p>
        </div>
        <?php if (!$invoice['dep_paid']): ?>
        <a href="payment.php?ref=<?= urlencode($invoice['ref']) ?>" class="btn btn--primary btn--sm">Pay Now →</a>
        <?php endif; ?>
      </div>
      <?php else: ?>
      <div style="background:#fff;padding:28px;text-align:center;border:1px solid #e0d8ce;">
        <p style="font-family:'MuseoModerno',sans-serif;font-weight:700;font-size:14px;color:#c4964a;">Pricing to be confirmed by our team within 24–48 hours.</p>
      </div>
      <?php endif; ?>

      <!-- Bank Transfer -->
      <div style="background:#ede9e1;padding:20px 24px;margin-top:24px;">
        <p style="font-family:'MuseoModerno',sans-serif;font-weight:700;font-size:10px;letter-spacing:.18em;text-transform:uppercase;color:#8a7d6e;margin-bottom:14px;">Bank Transfer Details</p>
  ...
  <div class="inv-tab-panel" style="display:none;">
    <div class="doc-wrap">
      <div class="doc-header">
        ...
        <div style="text-align:right;">
          <?php if ($invoice['dep_paid'] && $invoice['bal_paid']): ?>
          <span class="tag" style="background:#2a7a52;color:#fff;margin-bottom:10px;display:inline-block;">PAID IN FULL</span>
          <?php elseif ($invoice['dep_paid']): ?>
          <span class="tag tag--teal" style="margin-bottom:10px;display:inline-block;">DEPOSIT PAID</span>
          <?php else: ?>
          <span class="tag tag--gold" style="margin-bottom:10px;display:inline-block;">AWAITING PAYMENT</span>
          <?php endif; ?>
      ...
      <div class="doc-table-head" style="display:grid;grid-template-columns:1fr auto auto auto;">
        <span>Payment</span><span style="text-align:right;">Amount</span><span style="text-align:right;padding-left:16px;">Date</span><span style="text-align:right;min-width:80px;">Status</span>
      </div>

      <!-- Deposit row -->
      <div style="display:grid;grid-template-columns:1fr auto auto auto;padding:16px 20px;border-bottom:1px solid #f0ebe3;gap:16px;background:#fff;align-items:center;">
        <div>
          <p style="font-family:'MuseoModerno',sans-serif;font-weight:700;font-size:13px;color:#1a2118;">Deposit (30%)</p>
          <p style="font-family:'Museo',sans-serif;font-size:12px;color:#8a7d6e;">INV-<?= htmlspecialchars($invoice['ref']) ?>-D</p>
        </div>
        <p style="font-family:'MuseoModerno',sans-serif;font-weight:700;font-size:13px;color:#1a2118;text-align:right;">Rp <?= number_format($invoice['deposit'],0,',','.') ?></p>
        <p style="font-family:'Museo',sans-serif;font-size:12px;color:#8a7d6e;text-align:right;padding-left:16px;">
          <?php
          $dep_p = null;
          foreach ($db_payments as $p) { if ($p['payment_type']==='deposit') { $dep_p=$p; break; } }
          echo $dep_p && $dep_p['paid_at'] ? date('d M Y', strtotime($dep_p['paid_at'])) : '—';
          ?>
        </p>
        <p style="text-align:right;min-width:80px;">
          <?php if ($invoice['dep_paid']): ?>
          <span style="font-family:'MuseoModerno',sans-serif;font-weight:700;font-size:11px;color:#2cb896;">PAID ✓</span>
          <?php else: ?>
          <a href="payment.php?ref=<?= urlencode($invoice['ref']) ?>" style="font-family:'MuseoModerno',sans-serif;font-weight:700;font-size:11px;color:#c4964a;">Pay Now →</a>
          <?php endif; ?>
        </p>
      </div>

      <!-- Balance row -->
      <div style="display:grid;grid-template-columns:1fr auto auto auto;padding:16px 20px;border-bottom:1px solid #f0ebe3;gap:16px;background:#faf7f3;align-items:center;">
        <div>
          <p style="font-family:'MuseoModerno',sans-serif;font-weight:700;font-size:13px;color:#1a2118;">Balance (70%)</p>
          <p style="font-family:'Museo',sans-serif;font-size:12px;color:#8a7d6e;">INV-<?= htmlspecialchars($invoice['ref']) ?>-B</p>
        </div>
        <p style="font-family:'MuseoModerno',sans-serif;font-weight:700;font-size:13px;color:#1a2118;text-align:right;">Rp <?= number_format($invoice['balance'],0,',','.') ?></p>
        <p style="font-family:'Museo',sans-serif;font-size:12px;color:#8a7d6e;text-align:right;padding-left:16px;">
          <?php
          $bal_p = null;
          foreach ($db_payments as $p) { if ($p['payment_type']==='balance') { $bal_p=$p; break; } }
          echo $bal_p && $bal_p['paid_at'] ? date('d M Y', strtotime($bal_p['paid_at'])) : '—';
          ?>
        </p>
        <p style="text-align:right;min-width:80px;">
          <?php if ($invoice['bal_paid']): ?>
          <span style="font-family:'MuseoModerno',sans-serif;font-weight:700;font-size:11px;color:#2cb896;">PAID ✓</span>
          <?php elseif ($invoice['dep_paid']): ?>
          <a href="payment-balance.php?ref=<?= urlencode($invoice['ref']) ?>" style="font-family:'MuseoModerno',sans-serif;font-weight:700;font-size:11px;color:#c4964a;">Pay Now →</a>
          <?php else: ?>
          <span style="font-family:'MuseoModerno',sans-serif;font-size:11px;color:#8a7d6e;">Pending</span>
          <?php endif; ?>
        </p>
      </div>

      <?php if ($invoice['has_price']): ?>
      <div style="background:#faf7f3;padding:16px 20px;display:flex;justify-content:space-between;">
        <p style="font-family:'MuseoModerno',sans-serif;font-weight:800;font-size:15px;color:#1a2118;">Total</p>
        <p style="font-family:'MuseoModerno',sans-serif;font-weight:900;font-size:20px;color:<?= ($invoice['dep_paid'] && $invoice['bal_paid']) ? '#2a7a52' : '#1a2118' ?>;">
          Rp <?= number_format($invoice['total'],0,',','.') ?>
          <?php if ($invoice['dep_paid'] && $invoice['bal_paid']): ?> ✓<?php endif; ?>
        </p>
      </div>
      <?php endif; ?>
      ...
      <div style="text-align:center;padding:32px;background:#1a2118;margin-top:24px;">
        ...
        <p style="font-family:'Museo',sans-serif;font-size:13px;color:rgba(255,255,255,.4);margin-bottom:20px;">We hope your experience with Lombok Nature Culture was everything you imagined.</p>
        <div style="display:flex;gap:12px;justify-content:center;flex-wrap:wrap;">
          <a href="booking.php" class="btn btn--primary btn--sm">Plan Your Next Journey</a>
          <a href="https://wa.me/<?= SITE_WA ?>?text=<?= urlencode("Hi LNC! I'd like to leave a review. Ref: {$invoice['ref']}") ?>"
...
<script>
(function() {
  var btns   = document.querySelectorAll('.inv-tab-btn');
  var panels = document.querySelectorAll('.inv-tab-panel');

  function activate(idx) {
    btns.forEach(function(b, i) {
      b.style.color       = i === idx ? '#1a2118' : '#8a7d6e';
      b.style.borderBottom= i === idx ? '2px solid #1a2118' : '2px solid transparent';
    });
